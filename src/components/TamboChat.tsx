...
import { knownServices, lookupKnownService } from '@/lib/knownServices';
...
function RealTamboChat({ onSubmitFallback, onDetectedSubscriptions, onApiFailed }: TamboChatProps & { onApiFailed?: () => void }) {
 ...
 useEffect(() => {
  ...
  setLocalHistory(prev => {
   // Build a set of IDs already in local history for fast lookup
  ...
  *  Re-throws on terminal failure so the caller can rollback optimistic messages. */
  ...
  try {
   ...
   try {
    ...
    if (failCountRef.current >= MAX_API_FAILURES && onApiFailed) {
     ...
     onApiFailed();
     throw retryErr; // Re-throw so handleSubmit can rollback the optimistic message
    }
 ...
 const handleSubmit = useCallback(async (e: React.FormEvent) => {
  ...
  // Save user message to local history immediately (with a trackable ID for rollback)
  const localMsgId = `local_${Date.now()}`;
  setLocalHistory(prev => [...prev, { id: localMsgId, role: 'user' as const, text: displayText }]);
  setSubmitError(null);
  ...
  try {
   ...
   // doSubmit failed completely -- remove the optimistic user message to avoid ghost messages
   setLocalHistory(prev => prev.filter(m => m.id !== localMsgId));
  }
 ...
 const handleRetry = useCallback(async () => {
  ...
  setSubmitError(null);
  setValue(text);
  // Use rAF to ensure React has flushed the state update
 ...
 const displayMessages = useMemo(() => {
  ...
  const sdkIds = new Set(
   sdkVisible.map(m => m.id),
  );

  const localOnly = localHistory.filter(lm => !sdkIds.has(lm.id));
    ...
    <form onSubmit={handleSubmit}>
     ...
     <div className="flex items-center justify-between mt-2">
      <div className="flex items-center gap-1.5">
       ...
       {/* Fix 5: Demo mode does not support image uploads, so hide the attach button */}
      </div>
      <button
       type="submit"
...
// Known services imported from @/lib/knownServices (single source of truth)
...
// lookupKnownService imported from @/lib/knownServices (single source of truth)
...
function parseSubscriptionsFromMessage(message: string): Subscription[] {
 ...
 for (const rawName of names) {
  ...
  // Fix 4: Detect billing period per-subscription from nearby text, not the entire message
  const nearbyText = costMatch
   ? costMatch[0] + (costMatch.input?.slice((costMatch.index ?? 0) + costMatch[0].length, (costMatch.index ?? 0) + costMatch[0].length + 30) ?? '')
   : '';
  let isYearly: boolean;
  let isWeekly: boolean;
  if (nearbyText) {
   isYearly = /\b(?:year(?:ly)?|annual(?:ly)?|\/\s*yr)\b/i.test(nearbyText);
   isWeekly = /\b(?:week(?:ly)?|\/\s*wk)\b/i.test(nearbyText);
  } else if (names.length === 1) {
   // Fall back to checking the whole message only when there's a single subscription
   isYearly = /\b(?:year(?:ly)?|annual(?:ly)?|\/\s*yr)\b/i.test(message);
   isWeekly = /\b(?:week(?:ly)?|\/\s*wk)\b/i.test(message);
  } else {
   isYearly = false;
   isWeekly = false;
  }
