  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-800">
      ...
      <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center gap-4">
        <button
          onClick={onBack}
          className="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-600 flex items-center justify-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
        >
          <ArrowLeft className="w-4 h-4" />
        </button>
        <div className="flex-1">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">{subscription.name}</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Edit subscription details</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700">
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wide mb-1">Monthly Cost</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white">${costNum.toFixed(2)}</p>
          </div>
          <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700">
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wide mb-1">Next Renewal</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white">{renewalLabel}</p>
            {daysLeft !== null && (
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700">
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wide mb-1">Daily Avg</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white">${avgDailyCost}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Monthly Cost ($)</label>
            <input
              type="number"
              step="0.01"
              min="0"
              ...
              className="w-full p-3 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-200 dark:border-slate-600 text-sm font-medium text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="w-full p-3 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-200 dark:border-slate-600 text-sm font-medium text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all appearance-none cursor-pointer"
              >
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Billing Cycle</label>
              <select
                value={billing}
